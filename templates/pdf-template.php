...
<head>
	...
	<title><?php
		printf(
			...
			esc_html__( 'WP Snapshot — %s', 'wp-snapshot' ),
...
<body>
<button class="wps-print-btn" onclick="window.print()"><?php esc_html_e( 'Print / Save as PDF', 'wp-snapshot' ); ?></button>
<div class="wps-pdf">

	<!-- Cover Page -->
	<div class="wps-pdf__cover">
		<div class="wps-pdf__cover-logo">📋</div>
		<h1><?php printf( esc_html__( 'WP Snapshot', 'wp-snapshot' ) ); ?></h1>
		...
		<p class="wps-pdf__cover-meta">
			...
			<?php printf( esc_html__( 'Generated: %s', 'wp-snapshot' ), esc_html( $snapshot['generated_at'] ?? '' ) ); ?><br>
			<?php printf( esc_html__( 'WP Snapshot v%s', 'wp-snapshot' ), esc_html( $snapshot['generator_version'] ?? '' ) ); ?>
		</p>
	</div>

	<!-- Environment -->
	<section class="wps-pdf__section">
		<h2 class="wps-pdf__section-title"><?php esc_html_e( 'Environment', 'wp-snapshot' ); ?></h2>
		<div class="wps-pdf__section-body">
			<div class="wps-cards">
				...
				<div class="wps-card"><div class="wps-card__label"><?php esc_html_e( 'Database', 'wp-snapshot' ); ?></div><div class="wps-card__value" style="font-size:12px;"><?php echo esc_html( ( $env['db_type'] ?? '' ) . ' ' . ( $env['db_version'] ?? '' ) ); ?></div></div>
			...
			<table>
				<tbody>
					<tr><th><?php esc_html_e( 'Site URL', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $env['site_url'] ?? '' ); ?></td><th><?php esc_html_e( 'Home URL', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $env['home_url'] ?? '' ); ?></td></tr>
					<tr><th><?php esc_html_e( 'Server', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $env['server_software'] ?? '' ); ?></td><th><?php esc_html_e( 'OS', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $env['server_os'] ?? '' ); ?></td></tr>
					<tr><th><?php esc_html_e( 'PHP Memory', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $env['php_memory_limit'] ?? '' ); ?></td><th><?php esc_html_e( 'Max Upload', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $env['php_max_upload'] ?? '' ); ?></td></tr>
					<tr><th><?php esc_html_e( 'Timezone', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $env['wp_timezone'] ?? '' ); ?></td><th><?php esc_html_e( 'Locale', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $env['wp_locale'] ?? '' ); ?></td></tr>
					<tr><th>WP_DEBUG</th><td><?php echo ! empty( $env['wp_debug'] ) ? '<span class="badge badge--warning">On</span>' : 'Off'; ?></td><th><?php esc_html_e( 'Multisite', 'wp-snapshot' ); ?></th><td><?php echo ! empty( $env['is_multisite'] ) ? 'Yes' : 'No'; ?></td></tr>
				</tbody>
			</table>
		</div>
	</section>
	...
	<!-- Plugins -->
	<section class="wps-pdf__section">
		<h2 class="wps-pdf__section-title"><?php printf( esc_html__( 'Plugins — %d Active / %d Total', 'wp-snapshot' ), (int) ( $plug['active_count'] ?? 0 ), (int) ( $plug['total_plugins'] ?? 0 ) ); ?></h2>
		<div class="wps-pdf__section-body">
			<?php if ( ! empty( $plug['plugins'] ) ) : ?>
			<table>
				<thead><tr><th><?php esc_html_e( 'Name', 'wp-snapshot' ); ?></th><th><?php esc_html_e( 'Version', 'wp-snapshot' ); ?></th><th><?php esc_html_e( 'Author', 'wp-snapshot' ); ?></th><th><?php esc_html_e( 'Status', 'wp-snapshot' ); ?></th><th><?php esc_html_e( 'Update', 'wp-snapshot' ); ?></th></tr></thead>
				<tbody>
	...
	<!-- Themes -->
	<section class="wps-pdf__section">
		<h2 class="wps-pdf__section-title"><?php esc_html_e( 'Active Theme', 'wp-snapshot' ); ?></h2>
		<div class="wps-pdf__section-body">
			<?php $at = $themes['active_theme'] ?? []; ?>
			<table><tbody>
				<tr><th><?php esc_html_e( 'Name', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $at['name'] ?? '' ); ?></td><th><?php esc_html_e( 'Version', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $at['version'] ?? '' ); ?></td></tr>
				<tr><th><?php esc_html_e( 'Author', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $at['author'] ?? '' ); ?></td><th><?php esc_html_e( 'Type', 'wp-snapshot' ); ?></th><td><?php echo ! empty( $at['is_block_theme'] ) ? 'Block (FSE)' : 'Classic'; ?></td></tr>
				<tr><th><?php esc_html_e( 'Child Theme', 'wp-snapshot' ); ?></th><td colspan="3"><?php echo ! empty( $at['is_child_theme'] ) ? esc_html( $at['parent_theme'] ?? '' ) . ' (parent)' : 'No'; ?></td></tr>
			</tbody></table>
		</div>
	</section>
	...
	<!-- Database -->
	<section class="wps-pdf__section">
		<h2 class="wps-pdf__section-title"><?php esc_html_e( 'Database', 'wp-snapshot' ); ?></h2>
		<div class="wps-pdf__section-body">
			<div class="wps-cards">
				<div class="wps-card"><div class="wps-card__label"><?php esc_html_e( 'DB Size', 'wp-snapshot' ); ?></div><div class="wps-card__value" style="font-size:14px;"><?php echo esc_html( $db['total_db_size_human'] ?? '–' ); ?></div></div>
				...
				<div class="wps-card"><div class="wps-card__label"><?php esc_html_e( 'Autoloaded', 'wp-snapshot' ); ?></div><div class="wps-card__value" style="font-size:14px;"><?php echo esc_html( $db['autoload_size_human'] ?? '–' ); ?></div></div>
			...
			<table><tbody>
				<tr><th><?php esc_html_e( 'Prefix', 'wp-snapshot' ); ?></th><td><code><?php echo esc_html( $db['db_prefix'] ?? '' ); ?></code></td><th><?php esc_html_e( 'Charset', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $db['db_charset'] ?? '' ); ?></td></tr>
				<tr><th><?php esc_html_e( 'Orphaned Postmeta', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $db['orphaned_postmeta'] ?? 0 ); ?></td><th><?php esc_html_e( 'Trashed', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $db['trashed_count'] ?? 0 ); ?></td></tr>
			</tbody></table>
		</div>
	</section>
	...
	<!-- Security -->
	<section class="wps-pdf__section">
		<h2 class="wps-pdf__section-title"><?php esc_html_e( 'Security', 'wp-snapshot' ); ?></h2>
		<div class="wps-pdf__section-body">
			<table>
	...
	<!-- Cron -->
	<section class="wps-pdf__section">
		<h2 class="wps-pdf__section-title"><?php printf( esc_html__( 'Cron Jobs — %d events', 'wp-snapshot' ), (int) ( $cron['total_events'] ?? 0 ) ); ?></h2>
		<div class="wps-pdf__section-body">
			<?php if ( ! empty( $cron['events'] ) ) : ?>
			<table>
				<thead><tr><th><?php esc_html_e( 'Hook', 'wp-snapshot' ); ?></th><th><?php esc_html_e( 'Next Run', 'wp-snapshot' ); ?></th><th><?php esc_html_e( 'Schedule', 'wp-snapshot' ); ?></th></tr></thead>
				<tbody>
	...
	<section class="wps-pdf__section">
		<h2 class="wps-pdf__section-title"><?php esc_html_e( 'Post Types & Taxonomies', 'wp-snapshot' ); ?></h2>
		<div class="wps-pdf__section-body" style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
			<div>
				<strong><?php esc_html_e( 'Post Types', 'wp-snapshot' ); ?></strong>
				<table style="margin-top:6px;">
					<thead><tr><th><?php esc_html_e( 'Slug', 'wp-snapshot' ); ?></th><th><?php esc_html_e( 'Published', 'wp-snapshot' ); ?></th></tr></thead>
					<tbody>
			...
			<div>
				<strong><?php esc_html_e( 'Taxonomies', 'wp-snapshot' ); ?></strong>
				<table style="margin-top:6px;">
					<thead><tr><th><?php esc_html_e( 'Slug', 'wp-snapshot' ); ?></th><th><?php esc_html_e( 'Terms', 'wp-snapshot' ); ?></th></tr></thead>
					<tbody>
	...
	<!-- Users -->
	<section class="wps-pdf__section">
		<h2 class="wps-pdf__section-title"><?php printf( esc_html__( 'Users & Roles — %d total users', 'wp-snapshot' ), (int) ( $users['total_users'] ?? 0 ) ); ?></h2>
		<div class="wps-pdf__section-body">
			<table>
	...
	<!-- Media & Performance -->
	<section class="wps-pdf__section">
		<h2 class="wps-pdf__section-title"><?php esc_html_e( 'Media & Performance', 'wp-snapshot' ); ?></h2>
		<div class="wps-pdf__section-body" style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
			<table><tbody>
				<tr><th><?php esc_html_e( 'Attachments', 'wp-snapshot' ); ?></th><td><?php echo esc_html( number_format_i18n( $media['total_attachments'] ?? 0 ) ); ?></td></tr>
				<tr><th><?php esc_html_e( 'Uploads Size', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $media['upload_dir_size_human'] ?? '–' ); ?></td></tr>
				<tr><th><?php esc_html_e( 'Images', 'wp-snapshot' ); ?></th><td><?php echo esc_html( $media['mime_summary']['images'] ?? 0 ); ?></td></tr>
			</tbody></table>
			<table><tbody>
				<tr><th><?php esc_html_e( 'Object Cache', 'wp-snapshot' ); ?></th><td><?php echo ! empty( $perf['object_cache_active'] ) ? '<span class="badge badge--good">' . esc_html( $perf['object_cache_type'] ) . '</span>' : 'None'; ?></td></tr>
				<tr><th><?php esc_html_e( 'OPcache', 'wp-snapshot' ); ?></th><td><?php echo ! empty( $perf['opcache_enabled'] ) ? '<span class="badge badge--good">On</span>' : 'Off'; ?></td></tr>
	...
	<footer style="text-align:center;color:#646970;font-size:11px;margin-top:24px;padding-top:12px;border-top:1px solid #dcdcde;">
		<?php printf( esc_html__( 'Generated by WP Snapshot v%s · %s · %s', 'wp-snapshot' ), esc_html( $snapshot['generator_version'] ?? '' ), esc_html( $snapshot['site_url'] ?? '' ), esc_html( $snapshot['generated_at'] ?? '' ) ); ?>
	</footer>
